...
import { IRequestRenderParams, IRpcEngine, IAppState } from "../helpers/types";
...
export async function routeEthereumRequests(payload: any, state: IAppState) {
  if (!state.connector) {
    return;
  }
  ...
  if (!signingMethods.includes(payload.method)) {
    ...
    await signEthereumRequests(payload, state);
...
export async function signEthereumRequests(payload: any, state: IAppState) {
  ...
  if (connector) {
    ...
    if (result) {
      ...
      console.log("sign failed", message)
      if (!getAppControllers().wallet.isActive()) {
